...
api.mustValidate = async (input, validatorConfig, options = {}) => {
...
api.validateConfiguration = (validatorConfig, options = {}) => {
  return validate('veres-one-validator.config', validatorConfig);
};

api.validate = async (input, validatorConfig, options = {}) => {
  const {ledgerNode = null} = options;
  if(!ledgerNode) {
    throw new TypeError('"options.ledgerNode" must be a LedgerNode instance.');
  }
  ...
  input = await bedrock.jsonld.compact(input, {
    '@context': constants.VERES_ONE_CONTEXT_URL
  });
  console.log('COMPACTED', JSON.stringify(input, null, 2));
  await validate('veres-one-validator.operation', input);
  await _validateProofs(input, ledgerNode);
  if(input.type === 'CreateWebLedgerRecord') {
    return _validateCreateOperation({input, ledgerNode});
  }
  // must be an UpdateWebLedgerRecord op
  return _validateUpdateOperation({input, ledgerNode});
};
...
async function _validateCreateOperation({input, ledgerNode}) {
  await _nymCheck(input);
  let err;
  try {
    await ledgerNode.records.get({recordId: input.record.id});
  } catch(e) {
    err = e;
  }
  if(!err) {
...
async function _validateUpdateOperation({input, ledgerNode}) {
...
async function _validateProofs(input, ledgerNode) {
  /* Proofs enable:
