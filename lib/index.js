...
/**
 * Loads and returns the capabilityInvocation key pair from a did document.
 *
 * @throws {BedrockError} If capabilityInvocation key is missing or invalid
 *
 * @param didDocument {VeresOneDidDoc}
 * @returns {Promise<LDKeyPair|null>}
 * @private
 */
async function _invokeKey(didDocument) {
  let invokeKey;

  if(!didDocument.doc.capabilityInvocation) {
    throw new BedrockError(
  ...
  try {
    const keyData = didDocument.doc.capabilityInvocation[0].publicKey[0];
    invokeKey = await LDKeyPair.from(keyData);
  } catch(cause) {
    throw new BedrockError(
      `Invalid capabilityInvocation key: ${cause.message}`, 'ValidationError', {
        httpStatusCode: 400,
        public: true,
        cause
      });
...
async function _validateDid(input) {
  ...
  if(didDocument.isCryptonym()) {
    invokeKey = await _invokeKey(didDocument);
  }

  try {
    didDocument.validateDid({keyPair: invokeKey, env: cfg.environment});
  } catch(cause) {
    throw new BedrockError(
