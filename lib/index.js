...
bedrock.events.on('bedrock.start', () => {
  ...
  if(cfg.environment === 'live') {
    cryptonymDidRegex = /^did\:v1\:nym\:z.*/;
  } else {
    cryptonymDidRegex = /^did\:v1\:test\:nym\:z.*/;
  }
});
...
function _validateCreateOperation({input, ledgerNode}, callback) {
  async.auto({
    ...
    record: ['nymCheck', (results, callback) => ledgerNode.records.get(
      {recordId: input.record.id}, err => {
        if(!err) {
...
function _validateUpdateOperation({input, ledgerNode}, callback) {
  ...
  async.auto({
    record: callback => ledgerNode.records.get(
      {recordId: recordPatch.target}, callback),
...
function _createDidDocumentLoader(ledgerNode, record, jsonld) {
  ...
  return function(url, callback) {
    ...
    ledgerNode.records.get({recordId: did}, (err, result) => {
      let didDocument;
      if(err) {
        // if state machine does not have DID and it is to be created (i.e.
        // the record matches the DID), use it as the DID Document
