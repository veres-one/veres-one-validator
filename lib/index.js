...
const {config: {constants}, jsonld, util: {BedrockError}} = bedrock;
...
api.validate = async (
  {ledgerConfig = {}, ledgerNode, validatorInput, basisBlockHeight}) => {
  if(!ledgerNode) {
  ...
  // FIXME: remove different proof is provided for create/update, so moving
  // proof validation there.
...
async function _validateCreateOperation(
  ...
  const {record} = validatorInput;
  // TODO: currently need document loader to retrive key based on hash fragment
  // as well as controller document (record)

  // FIXME: remove for troubleshooting jsonld document loader
  // Object.freeze(record['@context']);

  const documentLoader = _createDidDocumentLoader(ledgerNode, record);
