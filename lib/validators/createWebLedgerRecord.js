...
module.exports = async ({
  ...
  if(!validatorInput.record.id.startsWith('did:v1:uuid:')) {
    const result = await _validateDid({
      basisBlockHeight, ledgerNode, validatorInput, validationParameterSet
    });
    if(!result.valid) {
      return result;
    }
...
async function _validateDid({
  ...
  console.log('TTTTT', didDocument);
  const {service} = didDocument.toJSON();
  if(service) {
    const {allowedServiceBaseUrl} = await helpers.getValidationParameterSet({
      basisBlockHeight, ledgerNode, validationParameterSet
    });
    if(allowedServiceBaseUrl) {
      console.log('PPPPPPPPPPPv', allowedServiceBaseUrl);
