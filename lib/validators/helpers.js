...
api.createDidDocumentLoader = ({
  basisBlockHeight, ledgerNode, record, recordPatch, ledgerConfig, operationType
}) => {
 ...
 *
 * @param {object} options - Options to use.
 * @param {string} options.operationType - An operationType such as
 ...
 */
async function _getDidDocument({
  operationType,
  basisBlockHeight,
  record,
  recordPatch,
...
/**
 * Gets the controller of a root zcap.
 * @private
 *
 ...
 * @param {string} options.operationType - An operationType such as
 *   `CreateWebLedgerRecord`.
 * @param {string} options.invocationTarget - The expected invocationTarget
 *   derived from the URL.
 * @param {object} options.record - A record being written to the ledger.
 * @param {object} options.recordPatch - A record that contains a json
 *   patch for an existing record.
 * @param {object} options.ledgerConfig - A legerConfig object.
 * @param {object} options.cryptoLd - A library for the decryption of a nym.
 * @param {object} options.verificationSuite - A jsigs Signature Suite.
 *
 * @returns {Promise<string>} The resulting controller of the root zcap.
 */
...
async function _getRootController({
  ...
  if(operationType === 'WebLedgerConfiguration') {
    const _ledgerConfig = await ledgerNode.config.get(
      {blockHeight: basisBlockHeight});
    const config = _ledgerConfig || ledgerConfig;
    return config.controller || config.ledger;
  }
...
/**
 * Gets a dynamically created root zcap.
 * @private
 *
 * @param {object} options to use.
 * @param {URL} options.url - The url of the root zcap.
 * @param {number} options.basisBlockHeight - The block height
 *   of the ledger to search on.
 * @param {object} options.ledgerNode - A ledgerNode instance.
 * @param {string} options.operationType - An operationType such as
 ...
 * @param {object} options.verificationSuite - A jsigs Signature Suite.
 *
 * @returns {Promise<object>} The resulting root zcap.
 */
async function _getRootZcap({
  url,
  basisBlockHeight,
  ledgerNode,
  operationType,
