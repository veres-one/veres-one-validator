...
api.createDidDocumentLoader = ({
  basisBlockHeight, ledgerNode, record, operationType
}) => {
  return async function(url) {
    ...
    if(operationType === 'CreateWebLedgerRecord' &&
      ...
      // FIXME: can this be removed now? has this been fixed?
...
async function _pluckDidNode(did, target, didDocument) {
  ...
  for(const property in didDocument) {
    ...
    for(const node of nodes) {
      if(node.id === target) {
        // FIXME: eliminate need to clone
        return bedrock.util.clone({
