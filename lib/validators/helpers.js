...
api.validateService = async ({
      ...
      return {
        error: new BedrockError(
          'Unable to retrieve ValidatorParameterSet.', 'UnknownError', {
            basisBlockHeight,
            // do not expose unknow error details to the user
            public: false,
            httpStatusCode: 400,
  ...
  const result = api.validateServiceEndpoints({allowedServiceBaseUrl, service});
  if(!result.valid) {
    return result;
  }
  // success
...
api.validateServiceEndpoints = ({allowedServiceBaseUrl, service}) => {
  let valid = false;
