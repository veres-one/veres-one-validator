...
module.exports = async ({
  ...
  if(originalDocument.id !== patchedDocument.id) {
    return {
      error: new BedrockError(
        'The DID document `id` is immutable.', 'ValidationError', {
          httpStatusCode: 400,
          public: true,
          originalDocument,
          patchedDocument,
  ...
  if(patchedDocument.type === 'ElectorPool') {
    ...
    // didDocument
    let result = validate('veres-one-validator.didDocument', patchedDocument);
    if(!result.valid) {
      return result;
    }
    ...
    const {service} = patchedDidDocument.toJSON();
    if(service) {
