...
module.exports = async ({basisBlockHeight, ledgerNode, validatorInput}) => {
  ...
  const documentLoader = helpers.createDidDocumentLoader(
    {basisBlockHeight, ledgerNode, record, operationType: validatorInput.type});
  ...
  if(!result.verified) {
    ...
    console.log('PROOF VALIDATION ERROR', JSON.stringify(error, null, 2));
    console.log('PROOF VALIDATION RESULT', JSON.stringify(result, null, 2));
    return {valid: false, error};
  }
  // success
  return {valid: true};
